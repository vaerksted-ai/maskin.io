...
A bet begins as a signal, not a guess. The raw material is feedback, a market move, churn risk, a competitive shift — evidence that something is worth pursuing. Before the bet exists, the insight behind it should be captured on its own — Maskin’s [discovery loop that turns signal into shaped bets](/marketplace/product-discovery/) is what makes that intake step continuous rather than a one-off exercise. In Maskin, this lineage is structural: the insight exists as an object before the bet, and the bet carries a reference back to it, so anyone later can trace why the work was chosen (Maskin architecture, 2026).
