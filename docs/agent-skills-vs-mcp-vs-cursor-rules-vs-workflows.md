...
The durable-knowledge sibling of a workspace skill — where compiled team memory sits so every skill reads the same thing before it acts — is [the compiled knowledge layer agents read first](/marketplace/knowledge-wiki/): closed bets and shipped decisions written as typed pages agents pick up on the same graph they already run on. How that compiled layer relates to query-time retrieval — and why the two are layers rather than rivals — is the [LLM wiki vs RAG](/docs/llm-wiki-vs-rag/) question.
