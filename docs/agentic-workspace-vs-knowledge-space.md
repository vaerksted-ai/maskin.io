...
## The diagnostic: can the system tell you whether the work succeeded?

There is a single diagnostic that separates a knowledge space from an agentic workspace, and it is the same one that separates a task tracker with AI summaries from an agentic workspace — the argument the [AI product management tool: not a task tracker](/docs/ai-product-management-tool/) piece runs against tier 2. It generalises cleanly to tier 3.
...
Read the two columns as tiers, not as a scoreboard. The knowledge space's column is correct for the tier it serves. The agentic workspace's column is what the tier past capture looks like when the object of work carries a win condition instead of a link count.
