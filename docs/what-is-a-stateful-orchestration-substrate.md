...
The structural claim underneath all five: **isolation moves into the agent; state moves out.** Each agent gets a fresh context so it can be spawned in parallel, retried, or swapped without side effects. State that survives across handoffs lives in a typed, queryable, external store — the substrate. A [shared, agent-readable wiki](/marketplace/knowledge-wiki/) is the concrete instance: a durable, interlinked artifact every agent reads before it answers, rather than state held inside one product. How that durable artifact relates to query-time retrieval — and why the two are layers rather than rivals — is the [LLM wiki vs RAG](/docs/llm-wiki-vs-rag/) question.
...
The substrate is one primitive in the stack, not the whole stack. Context engineering is the discipline. The **harness** is the layer around a single agent (state, tools, guardrails for one runtime). The **substrate** is what runs a pipeline of agents. A single step’s reusable capability is packaged as **[agent skills](/docs/learn/agent-skills/)** — discrete, loadable instructions that compose into a longer process only because the substrate carries state between the steps. All four coexist. LangChain’s Agent Inbox is the queue-shaped cousin to the graph-shaped substrate — same problem (routing agent work to humans), different geometry.
