...
There is a related boundary worth naming here, because it is the one teams trip over. MCP is the protocol layer — it governs *what an agent can call*, the typed tools and objects it reaches over the wire. What an agent *loads and executes* is a different kind of primitive: [agent skills](/docs/learn/agent-skills/) are reusable capability packages you author once, stored as a folder the agent reads into its own context when a task matches — no protocol call required, and no tie to a single model or integration. A tool is called over MCP; a skill is loaded into the agent. Both matter to an MCP-native design, and they are not the same thing.
For the workspace-scale version of these four markers — what “MCP-native” looks like when it’s a property of the whole product surface, plus the marketplace-vs-category and AI21 “workspace-as-sandbox” disambiguations — see the definitional companion: [MCP-native workspace: a category, not a checkbox](/docs/mcp-native-workspace/).
