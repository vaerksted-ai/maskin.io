...
Exposing the graph as MCP tools is the reach half of the category, and it is worth naming what it is not. A tool is what an agent calls; the reusable judgment about how and when to call it is a different primitive — one a team authors once and carries between agents. That is where [agent skills](/docs/learn/agent-skills/) sit: not an MCP server and not a tool, but packaged repeatable behaviour that an agent loads when a task, or a change in workspace state, matches its description.
...
The judgment loop is the moat, not the stack. MCP-native, self-hostable, Apache-2.0 are table stakes across a growing set of open-source entrants — the [self-hosted AI agent control plane](/alternatives/self-hosted-ai-agent-control-plane/) comparison walks the twelve 2026 coding-agent entrants that stack against a workspace-shaped product on exactly that vocabulary. What a workspace-shaped product owns is the surface on which humans stay on-the-loop rather than in-it.
